...
//   · 回答提问**不单独开工具**（每题一次额外工具没必要），但**必须用 send_message
//     发出去** —— 本项目的纯文本输出只是思考、不会进群（见 src/prompt.js 的工具
//     协议）。只有「开局 / 公布汤底 / 查进度」三个动作是本技能自己的工具。
//
// ── 边界 ─────────────────────────────────────────────────────────────────
...
export function promptSections(context = {}) {
  ...
  const lines = [
    ...
    '',
    '主持规则：',
    // ⚠️ 这条是**必须**的，不是啰嗦：本项目里纯文本输出只是"思考"，不会进群
    //    （见 src/prompt.js 的工具协议）。少了这一句，模型会把「是」写在正文里，
    //    群里什么都收不到 —— 用户看到的就是"思考（不发送）：是"。
    '· 【怎么回答】每次回答都要调用 send_message 真正发出去，例如 send_message("是")。只在正文里写「是」，群友是看不到的。',
    s.strictYesNo
...
function registerTools(a) {
  a.registerTool({
    ...
    async execute(ctx, args) {
      ...
      if (delivered.ok) {
        return {
          // "用 send_message 发出去"这句不能省：起手就讲清楚，模型才不会把「是」
          // 写在正文里（正文只是思考，不会进群 —— 见 src/prompt.js 的工具协议）。
          content: `汤面已经发到群里了，你别再重复念一遍。\n${secret}\n`
            + '现在开始当主持人：每次回答都调用 send_message 发出（例如 send_message("是")）——'
            + '只回「是 / 不是 / 无关 / 是也不是」，不要解释。'
        };
      }
      return {
