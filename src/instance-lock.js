...
/** 当前实例的标识（'' = 主实例）—— 账本署名、界面展示用它。 */
export function currentInstanceTag() {
  return PROFILE_ID;
}

/** 当前实例的显示名（`QQ_AGENT_INSTANCE_NAME`，为空时给个按实例号推出来的名字）。 */
export function currentInstanceName() {
  const explicit = String(process.env.QQ_AGENT_INSTANCE_NAME || '').trim();
  if (explicit) return explicit;
  return PROFILE_ID ? `实例 ${PROFILE_ID}` : '';
}
